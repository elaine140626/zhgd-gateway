...
public interface FaceGateApi extends Library {
	...
	//					FaceGateApi instance = Native.loadLibrary(FaceGateApi.class.getResource("").getPath().replaceFirst("/","").replaceAll("%20"," ")+"FaceGateApi.dll", FaceGateApi.class);
FaceGateApi instance = Native.loadLibrary("/home/tomcat9/webapps/Hujiang/WEB-INF/classes/org/lebo/facegate/libgatesdk.so", FaceGateApi.class);
	/**
	 * 初始化
	 * wListenPort    初始化端口
	 ...
	 */
	interface LogonAcceptCallback extends Callback {
		HANDLE getHandle();
	 ...
	 * 实时消息回调
	 */
	interface RecvMessageCallback extends Callback {
	...
	interface SearchCallback extends Callback {
		void setImageDir(String fileDir);
